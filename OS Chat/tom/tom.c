...
int main() {
	...
	while (1) {
		printf(">>>");
		fgets(parent, 80, stdin); // 키보드에서 문자열 입력, 최대 80개 문자
		write(fdw, parent, strlen(parent) + 1); // parent 배열의 문자열을 mkfifo에 쓰기
		if (strcmp(parent, "exit\n") == 0) {
			printf("break 합니다\n");
			break;
		}
		i = 0;
		while ((n = read(fdr, &child[i], 1)) > 0) { // mkfifo에서 읽어 child 배열에
			if (child[i] == '\0') break;
			if (i == 78) {
				child[++i] = '\0';
		...
		printf("child: %s\n", child);
		if (ctrl_c = signal(SIGINT, sigint_exit)) {
			printf("ctrl_c 이므로 break 합니다\n");
			break; // 루프 종료
		}
	}
	close(fdw);
